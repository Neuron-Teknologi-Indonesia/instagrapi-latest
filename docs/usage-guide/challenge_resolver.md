...
* Instagram sets `challenge.native_flow=true` on virtually every mobile checkpoint, including the ordinary `/challenge/{user_id}/{nonce}/` flow that exposes `select_verify_method` / `verify_email` steps; those are resolved through `challenge_code_handler` as usual. Only *opaque* native checkpoints — an encrypted (non-numeric) `/challenge/...` path together with a non-JSON `challenge_context` — are manual checkpoints: they do not expose an SMS/email/password step, so `challenge_code_handler` and `change_password_handler` will not be called for them and `ChallengeRequired` is raised immediately.
* A `challenge_required` payload without a `challenge.api_path` (for example the masked `Not Found` 404 returned on some comment endpoints) also raises `ChallengeRequired` instead of a `KeyError`.
* If the CAA login fallback is rate limited (HTTP 429), `login()` raises `PleaseWaitFewMinutes` / `ClientThrottledError` (with the legacy `BadPassword` attached as `__cause__`) instead of a misleading bad-password error. Back off before retrying; do not rotate proxies.
